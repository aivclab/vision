...
def add_mesh_square(arr, x, y, size):
  s = int(size / 2)
  xx, yy = np.mgrid[:arr.shape[0], :arr.shape[1]]
  return np.logical_or(arr, logical_and(
...
if __name__ == '__main__':
  img, mask = generate_random_data(24, 24, 3)

  input_images_rgb = [x.astype(np.uint8) for x in img]  # Change channel-order and make 3 channels for matplot
  target_masks_rgb = [helper.masks_to_color_img(x) for x in
                      mask]  # Map each channel (i.e. class) to each color
