...
class SkipHourglassFissionNet(nn.Module):
  """
  ...
  def parse_arguments(self, up_mode: str, merge_mode: str):
    ...
    if merge_mode in ('concat', 'add'):
      ...
      raise ValueError(f'"{up_mode}" is not a valid mode for merging up and down paths.'
  ...
  def __init__(self,
    ...
    if isinstance(output_channels, Dict):
      ...
      self.forks = {f'{self.default_prefix}{i}':s for i, s in enumerate(output_channels)}
