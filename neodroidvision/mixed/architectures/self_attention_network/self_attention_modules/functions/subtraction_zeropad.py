      ...
      for (int kw = 0; kw < ${kernel_w}; ++kw) {
        ...
        if (((h_out_s % ${stride_h}) == 0) && ((w_out_s % ${stride_w}) == 0)) {
          ...
          if ((h_out >= 0) && (h_out < ${top_height}) && (w_out >= 0) && (w_out < ${top_width})) {
            const int offset_top = ((n * ${input_channels} + c) * ${kernel_h} * ${kernel_w} + (kh *
            ${kernel_w} + kw)) * ${top_height} * ${top_width} + h_out * ${top_width} + w_out;
...
__all__ = ["SubtractionZeropad", "subtraction_zeropad"]
...
class SubtractionZeropad(Function):
    ...
    def forward(ctx, input, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel(
                "subtraction_zeropad_forward_kernel",
                _subtraction_zeropad_forward_kernel,
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                opt["nthreads"] = n
                f = load_kernel(
                    "subtraction_zeropad_input_backward_kernel",
                    _subtraction_zeropad_input_backward_kernel,
                    **opt
                )
                f(
