      ...
      for (int kw = 0; kw < ${kernel_w}; ++kw) {
        ...
        if ((h_in >= 0) && (h_in < ${bottom_height}) && (w_in >= 0) && (w_in < ${bottom_width})) {
          for (int cc = c; cc < ${input_channels}; cc += ${weight_channels}) {
            const int offset_bottom = ((n * ${input_channels} + cc) * ${bottom_height} + h_in) *
            ${bottom_width} + w_in;
...
__all__ = ["AggregationZeropad", "aggregation_zeropad"]
...
class AggregationZeropad(Function):
    ...
    def forward(ctx, input, weight, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel(
                "aggregation_zeropad_forward_kernel",
                _aggregation_zeropad_forward_kernel,
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                opt["nthreads"] = n
                f = load_kernel(
                    "aggregation_zeropad_input_backward_kernel",
                    _aggregation_zeropad_input_backward_kernel,
                    **opt
                )
                f(
            ...
            if ctx.needs_input_grad[1]:
                ...
                opt["nthreads"] = n
                f = load_kernel(
                    "aggregation_zeropad_weight_backward_kernel",
                    _aggregation_zeropad_weight_backward_kernel,
                    **opt
                )
                f(
