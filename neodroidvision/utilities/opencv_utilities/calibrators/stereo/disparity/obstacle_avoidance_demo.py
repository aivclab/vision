...
def obstacle_avoid():
    ...
    if numpy.sum(mask) / 255.0 > 0.01 * mask.shape[0] * mask.shape[1]:
        ...
        if cv2.contourArea(cnts[0]) > 0.01 * mask.shape[0] * mask.shape[1]:
            ...
            cv2.putText(
                output_canvas, "WARNING !", (x + 5, y - 40), 1, 2, (0, 0, 255), 2, 2
            )
            cv2.putText(
                output_canvas, "Object at", (x + 5, y), 1, 2, (100, 10, 25), 2, 2
            )
            cv2.putText(
                output_canvas,
                "%.2f cm" % depth_mean,
...
while True:
    ...
    if retL and retR:
        ...
        disparity = disparity.astype(numpy.float32)

        # Normalizing the disparity map
        disparity = (disparity / 16.0 - minDisparity) / numDisparities

        depth_map = M / (disparity + 1e-12)  # for depth in (cm)
