...
pathL = Path(".") / "data" / "stereoL"
pathR = Path(".") / "data" / "stereoR"
root = Path(".")
...
for i in tqdm(range(1, 28)):
    imgL = cv2.imread(str(pathL / f"img{i:d}.png"))
    imgR = cv2.imread(str(pathR / f"img{i:d}.png"))
    imgL_gray = cv2.imread(str(pathL / f"img{i:d}.png", 0))
    imgR_gray = cv2.imread(str(pathR / f"img{i:d}.png", 0))
...
cv_file = cv2.FileStorage(
    str(root / "data" / "stereo_rectify_maps.xml"), cv2.FILE_STORAGE_WRITE
)
