...
def read_data(dataset: SplitEnum = SplitEnum.training, path: Path = PROJECT_APP_PATH.user_cache / 'mnist')->Tuple:
  """
...
def convert_to_mnist_png(input_path, output_path)->None:
  ...
  for dataset in [SplitEnum.training, SplitEnum.testing]:
    write_dataset(*read_data(dataset, input_path),
                  output_path / dataset.value)
