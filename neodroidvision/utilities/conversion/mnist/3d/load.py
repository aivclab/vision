...
for i in range(10):
  ...
  layout = Layout(height=500, width=600, title=f"Digit: {str(digits[i][2])} in 3D space")
  fig = Figure(data=[Scatter3d(x=x_c, y=y_c, z=z_c, mode='markers',
                     marker={'size':12, 'color':z_c, 'colorscale':'Viridis', 'opacity':0.7})], layout=layout)
