...
def generate_perlin_noise(size=(50, 50)):
    noise = numpy.zeros(size)
    png = PerlinNoiseGenerator(2)
    for i in range(noise.shape[0]):
        for j in range(noise.shape[1]):
            noise[i, j] = png(i / noise.shape[0], j / noise.shape[1])
    return noise


if __name__ == "__main__":
    print(generate_perlin_noise())
