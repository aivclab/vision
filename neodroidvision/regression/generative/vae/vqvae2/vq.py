...
def vq_loss(inputs, embedded, commitment=0.25):
  ...
  """
    return torch.mean(
        torch.pow(inputs.detach() - embedded, 2)
...
class VQ(nn.Module):
  ...
  """
    ...
    def __init__(self, num_channels, num_latents, dead_rate=100):
        ...
        self.usage_count = nn.Parameter(
            dead_rate * torch.ones(num_latents).long(), requires_grad=False
        )
    ...
    """
        ...
        for i, count in enumerate(counts):
            ...
            if inputs_numpy is None:
                inputs_numpy = (
                    inputs.detach().cpu().numpy().reshape([-1, inputs.shape[-1]])
                )
    ...
    def _update_tracker(self, idxs):
        ...
        update = -numpy.ones([self.num_latents], dtype=numpy.int)
        for idx in raw_idxs:
            update[idx] = self.dead_rate
        self.usage_count.data.add_(
            torch.from_numpy(update).to(self.usage_count.device).long()
        )
