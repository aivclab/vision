...
"""
        ...
        for encoder in self.encoders:
            ...
            total_vq_loss = total_vq_loss + vq_loss(
                encoded, embedded, commitment=commitment
            )
        ...
        for i, decoder in enumerate(self.decoders):
            ...
            if target.dtype.is_floating_point:
                ...
                recon_loss = F.cross_entropy(
                    recon.view(-1, recon.shape[-1]), target.view(-1)
                )
...
"""
        ...
        for encoder, recon in zip(
            self.encoders[:-1][::-1], terms["reconstructions"][:-1]
        ):
        ...
        for i in range(hierarchy_size - 1):
            num_actual = i + 1
            dec_in = (
                terms["embedded"][-num_actual:][::-1] + layer_recons[num_actual - 1 :]
            )
