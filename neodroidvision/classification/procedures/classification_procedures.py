...
def test_model(model, data_iterator, latest_model_path, num_columns: int = 2):
    ...
    for i, a, b, c in zip(range(samples), input_images_rgb, y_pred_max, truth_labels):
        ...
        gd = ReportEntry(
            name=i,
            figure=img_html_embed_placeholder(
                format="jpg", size=[cell_width, cell_width]
            ),
    ...
    confusion_matrix = img_html_embed_placeholder(format="png", size=[800, 800])

    accuracy = math_markdown_embed("\dfrac{tp+tn}{N}"), None, accuracy_w
    precision = math_markdown_embed("\dfrac{tp}{tp+fp}"), precision_a, precision_w
    recall = math_markdown_embed("\dfrac{tp}{tp+fn}"), recall_a, recall_w
    f1_score = (
        math_markdown_embed("2*\dfrac{precision*recall}{precision+recall}"),
        fscore_a,
        fscore_w,
    )
    support = math_markdown_embed("N_{class_truth}"), support_a, support_w
    metrics = NOD.nod_of(
