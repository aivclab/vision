...
from samples.regression.reconstruction.vq_vae2.hierarchical.data import load_images
from samples.regression.reconstruction.vq_vae2.hierarchical.model import make_vae
...
def save_reconstructions(vae, images):
    ...
    with torch.no_grad():
        recons = [
            torch.clamp(x, 0, 1).permute(0, 2, 3, 1).detach().cpu().numpy()
    ...
    columns = numpy.concatenate([top_recons, real_recons, images], axis=-2)
    columns = numpy.concatenate(columns, axis=0)
    Image.fromarray((columns * 255).astype("uint8")).save(
        PROJECT_APP_PATH.user_data / "reconstructions.png"
    )
