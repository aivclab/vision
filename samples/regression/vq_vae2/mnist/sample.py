...
def main():
        ...
        for col in range(7):
            with torch.no_grad():
                outputs = torch.softmax(
                    generator(torch.from_numpy(inputs).to(get_torch_device())), dim=1
                )
    ...
    Image.fromarray((decoded * 255).astype(numpy.uint8)[0]).save(
        PROJECT_APP_PATH.user_data / "samples.png"
    )
