...
def main():
  ...
  Generator2 = torch.nn.Sequential(torch.nn.Linear(X_dim + z_dim, h_dim),
                                   torch.nn.ReLU(),
                                   torch.nn.Linear(h_dim, X_dim),
                                   torch.nn.Sigmoid()
                                   )
  ...
  with TensorBoardPytorchWriter(PROJECT_APP_PATH.user_log / str(time.time())) as writer:
