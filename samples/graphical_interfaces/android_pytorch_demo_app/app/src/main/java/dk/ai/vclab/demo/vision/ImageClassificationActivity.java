package dk.ai.vclab.demo.vision;
...
import dk.ai.vclab.demo.Constants;
import dk.ai.vclab.demo.R;
import dk.ai.vclab.demo.Utils;
import dk.ai.vclab.demo.vision.view.ResultRowView;
...
public class ImageClassificationActivity extends AbstractCameraXActivity<ImageClassificationActivity.AnalysisResult> {
  ...
  static class AnalysisResult {
    ...
    public AnalysisResult(String[] topNClassNames, float[] topNScores, long moduleForwardDuration, long analysisDuration) {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    final ResultRowView headerResultRowView = findViewById(R.id.image_classification_result_header_row);
  ...
  protected TextureView getCameraPreviewTextureView() {
    return ((ViewStub) findViewById(R.id.image_classification_texture_view_stub)).inflate().findViewById(R.id.image_classification_texture_view);
  }
  ...
  protected AnalysisResult analyzeImage(ImageProxy image, int rotationDegrees) {
    ...
    try {
      ...
      runOnUiThread(() -> {
        if (!isFinishing()) {ImageClassificationActivity.this.finish();
        }
      });
      return null;
    }
  ...
  protected void applyToUiAnalyzeImageResult(AnalysisResult result) {
    ...
    for (int i = 0; i < TOP_K; i++) {
      ...
      rowView.scoreTextView.setText(String.format(Locale.US, SCORES_FORMAT, result.topNScores[i]));
  ...
  protected String getModuleAssetName() {
    mModuleAssetName = "mobilenet_quantized_scripted_925.pt";
