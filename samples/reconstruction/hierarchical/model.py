...
from vision.reconstruction.vqvae2.attention import PixelAttention
from vision.reconstruction.vqvae2.pixel_cnn import (ChannelNorm,
                                                    PixelCNN,
                                                    PixelConvA,
                                                    PixelConvB,
                                                    )
from vision.reconstruction.vqvae2.vq_vae import (HalfDecoder,
                                                 HalfEncoder,
                                                 HalfQuarterDecoder,
                                                 QuarterEncoder,
                                                 VQVAE,
...
class TopPrior(nn.Module):
  def __init__(self, depth=128, num_heads=2):
    ...
    self.pixel_cnn = PixelCNN(PixelConvA(depth, depth),
    ...
    self.out_stack = nn.Sequential(nn.Conv2d(depth * 2, depth, 1),
                                   Residual1x1(depth),
                                   Residual1x1(depth),
                                   Residual1x1(depth),
                                   Residual1x1(depth),
