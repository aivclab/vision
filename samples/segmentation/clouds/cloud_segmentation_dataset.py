...
# -*- coding: utf-8 -*-
import draugr
from draugr.visualisation.strings import resize_image_cv
...
class CloudSegmentationDataset(Dataset):
  ...
  def __init__(self,
    ...
    if subset == 'train':
      self.transforms = albumentations.Compose(self.training_augmentations() #+
  ...
  def fetch_masks(self,
    ...
    for idx, im_name in enumerate(df["im_id"].values):
      for classidx, classid in enumerate(self.categories.values()):
        mpath = str(self.base_image_data / 'train_masks_525' / f'{classid}{im_name}')
