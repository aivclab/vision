...
from draugr.torch_utilities import torch_seed, get_torch_device
import cv2
...
def train_d(model,
  ...
  for epoch in E:
    ...
    for data, target in train_set:
      data, target = data.to(get_torch_device()), target.to(get_torch_device())
      optimizer.zero_grad()
      output, *_ = model(data)
