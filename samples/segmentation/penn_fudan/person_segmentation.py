...
def reschedule_learning_rate(
    ...
    #   #TODO: basen on total num epochs
    ...
    *,
    ...
    for epoch_i in E:
        ...
        with TorchTrainSession(model):
            for data, target in progress_bar(train_loader):
        ...
        with TorchEvalSession(model):
            with torch.no_grad():
                for data, target in progress_bar(valid_loader):
                    (
                        output,
                        *_,
                    ...
                    writer.scalar(
                        "dice_validation",
                        dice_loss(output, target.to(global_torch_device()).float()),
                    )
...
def main(
    ...
    load_previous_model_model: bool = True,
    writer: Writer = TensorBoardPytorchWriter(
        PROJECT_APP_PATH.user_log / "person_segmentation" / f"{time.time()}"
    ),
):
    """description"""
    ...
    if load_previous_model_model and save_model_path.exists():
...
if __name__ == "__main__":
    main()
    # main(train_model=True, load_previous_model_model=False)
