...
from draugr.torch_utilities.initialisation.seeding import seed
...
class CloudDataset(Dataset):
  def __init__(self,
    ...
    self.df = df
    if datatype != "test":
      self.data_folder = f"{resized_loc}/train_images_525"
    else:
      self.data_folder = f"{resized_loc}/test_images_525"
...
def draw_masks(img2, img_mask_list):
  ...
  for ii in range(4):  # for each of the 4 masks
    ...
    if ii < 3:  # use different color for each mask
      ...
      color_mask[:, :, 0], color_mask[:, :, 1], color_mask[:, :,
                                                2] = temp_mask, temp_mask, temp_mask  # broadcasting to 3
      # channels
...
def get_img(x, folder: str = str(resized_loc/"train_images_525")):
  """
...
def reschedule():
  ...
  if epoch == 7:
    ...
    current_lr = [param_group['lr'] for param_group in optimizer.param_groups][0]
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 6, eta_min=current_lr / 100,
                                                           last_epoch=-1)
  if epoch == 13:
    optimizer = torch.optim.SGD(model.parameters(), lr=0.005)
    current_lr = [param_group['lr'] for param_group in optimizer.param_groups][0]
...
pathlist = ["../input/understanding_cloud_organization/test_images/" + i.split("_")[0] for i in
            subm['Image_Label']]
